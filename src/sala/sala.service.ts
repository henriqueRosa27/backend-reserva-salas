...
export class SalaService {
  ...
  async findById(id: number): Promise<SalaEntity> {
    ...
    throw new HttpException({ erro: 'Sala não existe' }, HttpStatus.NOT_FOUND);
  }
  ...
  async checkName(nome: string, idPredio: number, id?: number): Promise<void> {
    let query = this.rep
      ...
      .where('sala.nome=:NOME', { NOME: nome })
      .andWhere('sala.predio_id=:IDPREDIO', { IDPREDIO: idPredio });
    ...
    if (result) {
      throw new HttpException(
        { erro: 'Sala já existe para esse predio!' },
